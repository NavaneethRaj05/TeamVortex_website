    ...
    return (
        ...
        >
            ...
            >
                ...
                {/* Events List — each card clicks to /contests */}
                    ...
                    <div className="space-y-4">
                        {events.slice(0, 3).map((event, index) => (
                            <motion.button
                                ...
                                className="w-full text-left group p-4 rounded-xl bg-white/5 border border-white/10 hover:border-vortex-blue/40 hover:bg-white/10 active:bg-vortex-blue/10 transition-all duration-200 touch-manipulation"
                            >
                                ...
                                <div className="space-y-1.5 text-sm text-white/50">
                                    <div className="flex items-center gap-2">
                                        <Calendar className="w-4 h-4 text-vortex-blue flex-shrink-0" />
                                        <span>{new Date(event.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                                    </div>
                                    {event.location && (
                                        <div className="flex items-center gap-2">
                                            <MapPin className="w-4 h-4 text-vortex-blue flex-shrink-0" />
                                            <span>{event.location}</span>
                                        </div>
                                    )}
                                </div>
                                <div className="flex items-center gap-1 mt-2 text-xs text-vortex-blue font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <div className="p-6 border-t border-white/10 flex items-center justify-center bg-black/40">
                    <Link to="/contests" onClick={onClose}
